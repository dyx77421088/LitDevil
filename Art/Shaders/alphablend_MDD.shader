Shader "XuanFu/Particles/alphablend_MDD"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{ 
				...
				half2 uv_DistMask=TRANSFORM_TEX(i.uv.xy, _DistMask);
				...
				float2 uv_dist1 = i.uv.xy*_DisturbanceTex_ST.xy *- _DisturbanceTex_ST.zw-float2(0.3,0.7) - float2(_DistSpeed_x*_Time.y*0.7, _DistSpeed_y*_Time.y*0.5);
				uv = uv + max(tex2D(_DisturbanceTex, uv_dist).r,tex2D(_DisturbanceTex, uv_dist1).r)*_Disturbance_Pow*tex2D(_DistMask, uv_DistMask).r + float2(_MainTexSpeed_x*_Time.y, _MainTexSpeed_y*_Time.y)+float2 (i.uv1.x, i.uv1.y);
